...
CREATE TABLE CDR_FILE 
   (	CDR_FILE_ID bigint identity(1,1) primary key, 
      ...
      FILE_SEQ bigint, 
      FILE_DATE DATE, 
      PROCESS_DATE DATETIME default getdate(), 
      DATASOURCE int, 
      FILE_SIZE float, 
      TOTAL_RECORDS int, 
      ACTION VARCHAR(30 ), 
...
select *From cdr_file;

create sequence CDR_FILE_SEQ as bigint start with 1 increment by 1 minvalue 1 maxvalue 999999999 no cycle no cache;

select next value  for cdr_seq;

select *From threads_log;

select *From CDR_LOG_DETAIL;
delete from threads_log;

delete from cdr_file;
delete from CDR_LOG_DETAIL;
...
select *From cdr_file where process_date is not null;
select *From CDR_LOG_DETAIL where process_date is  null;
